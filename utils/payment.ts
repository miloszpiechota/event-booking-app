// src/api/payment.ts
import { getUserToken, getUserId } from '../utils/session.ts'; // lub inna ścieżka

import { updateTicketQuantity } from "../utils/updateTicketQuantity.ts"; // Import funkcji aktualizującej bilety
...
export async function handlePayment(params: PaymentParams): Promise<void> {
  try {
    ...
    const supabaseFunctionDomain = Constants.expoConfig?.extra?.SUPABASE_FUNCTION_DOMAIN;
